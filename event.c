	...
	for (i=0; i<MAX_EVENT; i++)
		if (timer_events[i].pfunc != NULL)
			if ((timer_events[i].cmp_val - msTicks) > 0x80000000) {
				...
				pfunc();
				//
				// Ётот возврат требуетс€, чтобы не перезатиралс€ флаг отправки пакета, 
				// если 2 событи€ происход€т в один момент. ¬озврат позвол€ет отсрочить 
				// обработку на следующее таймерное прерывание.
				//
				return;
			}
}
...
void event_init(void) {
	...
	if (SysTick_Config(SystemCoreClock / (1000/2))) // Init SysTick 2ms
		while (1);
}
